import { Link, useParams } from "react-router-dom";import products from "../data/kada.json";
import ProductCategoryCard from "../components/ProductCard";

export default function BanglesDesignerKada() {
  ...
  const filtered = name
    ? products.filter(
      (p) =>
        p.BanglesDesignerKada?.toLowerCase() === name.toLowerCase()
    )
    : products;
    ...
    <>
      ...
      <section className="bg-img1 txt-center p-lr-15 p-tb-92">
        <h2 className="ltext-105 cl0 txt-center">
          {name ? name : "Bangles Designer Kada"}
        </h2>
      </section>
